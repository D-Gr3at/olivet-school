...
//  $('#closure_date').empty();
 var total_credit_load = 0;	
// $(document).on("change", "#semester", function(){
// 	var semester = $("#semester").val();
// 	var option_id = $("#department_option_id").val();
...
// 		type: "POST",
// 		url: "utilities.php",
// 		data: {op:"CourseRegistration.getSemesterCourses", semester:semester, level:level, student_id:student_id, department_id:department_id, department_option_id:option_id},
// 		success: function (response) {
// 			var data = JSON.parse(response);
// 			$('#compulsory_courses_section').empty();
// 			$('#elective_courses_section').empty();
// 			$("#total_unit").empty();
// 			$("#submitBtn").empty();
// 			$('#message').empty();
// 			$('#closure_date').empty();
...
// 				var compulsory_courses = data.compulsory;
// 				var elective_courses = data.elective;
// 				var closure_date = data.closure_date;
// 				if (compulsory_courses != null){
// 					if(!(new Date(Date.now()) > new Date(closure_date[0].closure_date))){
// 						swal({
// 							title: "Notice!",
// 							icon: "warning",
// 							text: `Course Registration ends: ${new Date(closure_date[0].closure_date).toDateString()}.`,
// 							button:{
// 								text: "OK",
// 								value: true,
// 								visible: true,
...
// 						});
// 						total_credit_load = 0;
// 						compulsory_courses.forEach((element, index) => {
// 							total_credit_load += parseInt(element.course_unit, 10);
// 							$('#compulsory_courses_section').append(
// 								`<tr class="d-flex">`+
// 									`<td class="col-1">${serialNumber}</td>`+
// 									`<input type="hidden" name="compulsory_course_id[]" id="course_id" value="${element.course_id}" class="form-control text-center"/>`+
// 									`<td class="col-6"><input type="text" name="course_title[]" id="course_title${index}" value="${element.course_title}" class="form-control" readonly/></td>`+
// 									`<td class="col-2"><input type="text" name="course_code[]" id="course_code${index}" value="${element.course_code}" class="form-control text-center" readonly/></td>`+
// 									`<td class="col-2"><input type="text" name="course_unit[]" id="course_unit${index}" value="${element.course_unit}" class='form-control text-center' readonly/></td>`+
// 									`<td class="col-1"></td>`+
// 								`</tr>`
// 						   );
// 						   serialNumber++;
// 						});
// 						$("#submitBtn").empty();
// 						$("#submitBtn").append("<input type='submit' value='Register' class='btn btn-md btn-primary'>");
// 					}else{
// 						swal({
// 							title: "Notice!",
// 							icon: "error",
...
// 						});
// 						$('#compulsory_courses_section').empty();
// 						$('#compulsory_courses_section').append(`<div class="d-flex justify-content-center"><br><span class='display-4 mt-3' style='font-size: 1.5em'>No available courses </span></div>`);
// 					}					
// 				}else{
// 					$('#compulsory_courses_section').empty();
// 					$('#compulsory_courses_section').append(`<div class="d-flex justify-content-center"><br><span class='display-4 mt-3' style='font-size: 1.5em'>No available courses </span></div>`);				
// 				}
// 				if(elective_courses != null && !(new Date(Date.now()) > new Date(closure_date[0].closure_date))){
// 					elective_courses.forEach((element, index) => {
// 						$('#elective_courses_section').append(
// 							`<tr class="d-flex">`+
// 								`<td class="col-1">${serialNumber}</td>`+
// 								`<input type="hidden" name="elected_course_id[]" id="course_id" value="${element.course_id}" class="form-control text-center"/>`+
// 								`<td class="col-6"><input type="text" name="elected_course_title[]" id="course_title${index}" value="${element.course_title}" class="form-control" readonly/></td>`+
// 								`<td class="col-2"><input type="text" name="elected_course_code[]" id="course_code${index}" value="${element.course_code}" class="form-control text-center" readonly/></td>`+
// 								`<td class="col-2"><input type="text" name="elected_course_unit[]" id="course_unit${index}" value="${element.course_unit}" class='form-control text-center' readonly/></td>`+
// 								`<td class="col-1"><input type="checkbox" id="elected_course${index}" class="elected_course" onclick="changeUnit(${element.course_unit},${index})"/></td>`+
// 								`<input type="hidden" name="elected_course[]" id="elected_course_value${index}" value="No">`+
// 							`</tr>`
// 						);
// 						serialNumber++;
// 					});
...
// 				}else{
// 					$('#elective_courses_section').empty();
// 					$('#elective_courses_section').append(`<div class="d-flex justify-content-center"><span class='display-4 mt-3' style='font-size: 1.5em'>No available courses </span></div>`);				
// 				}
// 				$("#total_unit").empty();
// 				$("#total_unit").append("<span style='font-size: 1.2em'>Total Credit Unit: <span id='tcl'>"+`${total_credit_load}`+"</span></span>");
// 			}else{
...
$("#courseRegForm").submit(function(){
	let data = $("#courseRegForm").serialize();
	let total_credit_load = $("#tcl").text();
	$.ajax({ 
		type: "post", 
		url: "utilities.php", 
		data: "op=CourseRegistration.saveCourseRegistration&data="+data+"&total_credit_load="+total_credit_load,
		success: function(response){ 
...
$(document).on("click","#viewBtn" , function(){
	let data = $("#viewCourseForm").serialize();
	// console.log(data);

	$.ajax({
		type: "GET", 
		url: "utilities.php",
		data: "op=CourseRegistration.getRegisteredCourses&data="+data,
		success: function(response){ 
			let data = JSON.parse(response);
			// console.log(data);
			
			total_credit_load = 0;
			$('#compulsory_section').empty();
			$('#elective_section').empty();
			$('#elective_null').empty();
			$("#total_unit").empty();
			$('#compulsory_null').empty();
			$('#printBtn').empty();
			let serialNumber = 1;
			if(data.response_code == 0){
				let compulsory_courses = Object.values(data.compulsory_courses);
				let elective_courses = Object.values(data.elective_courses);
				if(compulsory_courses.length != 0){
					// console.log(compulsory_courses);
					
					compulsory_courses.forEach((element, index) =>{
						total_credit_load += parseInt(element.course_unit, 10);
						$('#compulsory_section').append(
							`<tr class="d-flex">`+
									`<td class="col-1">${serialNumber}</td>`+
									`<input type="hidden" name="compulsory_course_id[]" id="course_id" value="${element.course_id}" class="form-control text-center"/>`+
									`<td class="col-6"><input type="text" name="course_title[]" id="course_title${index}" value="${element.course_title}" class="form-control" readonly/></td>`+
					...
					$('#printBtn').empty();
					$('#printBtn').append("<input type='button' id='printForm' value='Print'  class='btn btn-md btn-primary printBtn'/>");
				}else{
					$('#compulsory_section').empty();
					$('#compulsory_section').append("<div class='d-flex justify-content-center'><span class='display-4 mt-3' style='font-size: 1.5em'>No available courses </span></div>");
				}
				if(elective_courses.length != 0){
					...
					elective_courses.forEach((element, index) =>{
						total_credit_load += parseInt(element.course_unit, 10);
						$('#elective_section').append(
							`<tr class="d-flex">`+
								`<td class="col-1">${serialNumber}</td>`+
								`<input type="hidden" name="elected_course_id[]" id="elected_course_id" value="${element.course_id}" class="form-control text-center"/>`+
								`<td class="col-6"><input type="text" name="elected_course_title[]" id="elected_course_title${index}" value="${element.course_title}" class="form-control" readonly/></td>`+
								`<td class="col-2"><input type="text" name="elected_course_code[]" id="elected_course_code${index}" value="${element.course_code}" class="form-control text-center" readonly/></td>`+
								`<td class="col-2"><input type="text" name="elected_course_unit[]" id="elected_course_unit${index}" value="${element.course_unit}" class='form-control text-center' readonly/></td>`+
								`<td class="col-1"></td>`+
							`</tr>`
						);
						serialNumber++;
					...
					$('#elective_section').empty();
					$('#elective_section').append("<div class='d-flex justify-content-center'><span class='display-4 mt-3' style='font-size: 1.5em'>No available courses </span></div>");
				}
				$("#total_unit").empty();
				$("#total_unit").append("<span style='font-size: 1.2em'>Total Credit Unit: <span id='tcl'>"+`${total_credit_load}`+"</span></span>");
			}else{
...
$(document).on("click", "#printForm", function(){
	let data = $("#viewCourseForm").serialize();
	window.open("generate_registered_courses.php?"+data, '_blank');
	// window.location = "generate_registered_courses.php?"+data;
});