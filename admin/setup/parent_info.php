...
if(isset($_REQUEST['surname_id']))
{
    ...
    $sql_family_unit = "SELECT family_id,family_head FROM family WHERE family_name = '$id' AND family_id <> '$fam_id' ";
...
// if we find any
$sql = "SELECT parent_id FROM family WHERE family_id = '$fam_id' LIMIT 1";
...
$fam_head = strtoupper($dbobject->getitemlabel('family','family_id',$fam_id,'family_head'));
...
<div class="modal-header">
    <h4 class="modal-title" style="font-weight:bold">Enter parent for <?php echo $fam_head; ?></h4>
...
<div class="modal-body m-3">
    <form id="form21" onsubmit="return false">
       ...
       <input type="hidden" name="op" value="Family.updateFamilyParent">
       ...
       <div class="row">
        ...
        <button id="save_facility" onclick="saveRecords()" class="btn btn-info sw-btn-next mb-1">Save and Continue</button>
        <br>
    ...
    <p><small style="color:red;font-weight: bold;"><span style="color:#000"> NOTE:</span> Close this form and click on "Create Family" button if you can't find <?php echo $fam_head ?>'s parent from the dropdown.</small></p>
</div>
<script>
    function saveRecords()
    {
        $("#save_facility").text("Loading......");
        var dd = $("#form21").serialize();
                    ...
                    if($("#proceed").is(":checked"))
                        {
                            ...
                            var parent_id = re.data.fam_id;
                            var fam_id = "<?php echo $fam_id ?>";
                            getpage('setup/grand_parent_info.php?surname_id='+surname+'&parent_fam_id='+parent_id+'&fam_id='+fam_id,'modal_div');
                        }
                }
            else
                alert(re.response_message)
