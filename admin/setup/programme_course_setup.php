...
{
    $programme_course_id  = $_REQUEST['programme_course_id'];
    $programme_course = $dbobject->db_query("SELECT * FROM programme_course_setup_tbl WHERE programme_course_id=".$programme_course_id."");
    $department = $dbobject->db_query("SELECT * FROM department_setup_tbl WHERE dapartment_id=".$programme_course[0]['department_id']."");
    $department_option = $dbobject->db_query("SELECT * FROM programme_setup WHERE programme_id=".$programme_course[0]['programme_id']."");
    $faculty = $dbobject->db_query("SELECT * FROM faculty_settup WHERE faculty_id=".$programme_course[0]['faculty_id']."");
    $course_setup = $dbobject->db_query("SELECT * FROM course_setup_tbl WHERE programme_course_fk =".$programme_course_id."");
    $operation = 'edit';
...
<div class="modal-header">
    <h4 class="modal-title" style="font-weight:bold"><?php echo ($operation=="edit")?"Edit ":""; ?>Programme Course Setup<div><small style="font-size:12px">All asterik fields are compulsory</small></div></h4>
...
<div class="modal-body m-3 ">
    <form id="form1" onsubmit="return false">
       <input type="hidden" name="op" value="ProgrammeCourse.saveProgrammeCourse"/>
       <input type="hidden" name="operation" value="<?php echo $operation; ?>"/>
       <input type="hidden" name="programme_course_id" value="<?php echo $programme_course_id; ?>"/>
       ...
       <input type="hidden" name="posted_by" id="posted_by" value="<?php echo $_SESSION['firstname_sess'].' '.$_SESSION['lastname_sess']; ?>"/>
        ...
        <fieldset class="form-group">
            ...
            <div class="row">
                <?php if($course_setup != NULL){ 
                    foreach($course_setup as $key => $value){?>
                <div class="container">
                    ...
                    <div class="row">
                        <input type="hidden" name="course_id[]" value="<?php echo empty($value['course_id'])?$course_id:$value['course_id']; ?>" class="form-control"/>
                        <div class="col-sm-7">
                            <div class="form-group">
                                <label class="form-label">Course Title<span class="asterik">*</span></label>
                                <input type="text" name="course_title[]" value="<?php echo $value['course_title']; ?>" class="form-control"/>
                            </div>
                        </div>
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label class="form-label">Course Code<span class="asterik">*</span></label>
                                <input type="text" name="course_code[]" value="<?php echo $value['course_code']; ?>" class="form-control"/>
                            </div>
                        </div>
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label class="form-label">Duration <small class="text-danger"><em>(hours)</em></small><span class="asterik">*</span></label>
                                <input type="text" name="course_duration[]" value="<?php echo $value['course_duration']; ?>" class="form-control"/>
                            </div>
                        </div>
                        <div class="col-sm-1">
                            <div class="form-group">
                                <label class="form-label">Unit<span class="asterik">*</span></label>
                                <input type="text" name="course_unit[]" value="<?php echo $value['course_unit']; ?>" class="form-control"/>
                            </div>
                        </div>
                    </div>
                </div>
                <?php }
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-1">
                            <div class="form-group">
                                <label class="form-label">Unit<span class="asterik">*</span></label>
                                <input type="text" name="course_unit[]" class="form-control"/>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <input type="submit" id="save_programme_course" class="btn btn-primary" value="Submit"/>
    </form>
</div>
</div>
<script>
    $(document).ready(function() {
        ...
        $("#form1").on('submit', function() {
            $("#save_programme_course").val("Loading......");
            var dd = $("#form1").serialize();
            // console.log(dd);

            $.post("utilities.php",dd,function(re){
                // console.log(re);
                $("#save_programme_course").val("Save");
