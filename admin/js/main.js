...
$(document).on("change", "#faculty", function(){
	...
	$.ajax({
		type: "POST",
		url: "utilities.php",
		data: {op:"Curriculum.getDepartments",id:data},
		success: function (response) {
			...
			if(data.response_code == 0){
				...
				if (response_data != null){
					response_data.forEach((element, index) => {
						$('#department').append("<option value='"+element.dapartment_id+"'>"+element.department_name+"</option>");
						
					});
				}else{
					$('#department').append("<option value=''>::NO DEPARTMENT TO SELECT::</option>");				}
			}else{
...
$(document).on("change", "#department", function(){
	...
	$.ajax({
		type: "POST",
		url: "utilities.php",
		data: {op:"Curriculum.getProgramme", id:data},
		success: function (response) {
			var data = JSON.parse(response);			
			if(data.response_code == 0){
	...
	$.ajax({
		...
		success: function (response) {
			...
			if(data.response_code == 0){				
				...
				if (response_data != null){
					response_data.forEach((element, index) => {
						myElement.innerHTML += "<option value='"+element.programme_id+"'>"+element.programme_name.toUpperCase()+"</option>";
					});
				}else{
					myElement.innerHTML = "<option value=''>::NO PROGRAMME TO SELECT::</option>";
				}
			}else{
...
$(document).on("change", ".programme", function(){
	...
	var parent = this.parentNode.parentNode.nextElementSibling;
	var myElement = parent.querySelector(".course_code_class");
	// console.log(parent);

	if(!level){
		$("#server_mssg").text("Please select level");
		$("#server_mssg").css({'color':'red','font-weight':'bold'});
		return;
	}
	if(!semester){
		$("#server_mssg").text("Please select semester");
		$("#server_mssg").css({'color':'red','font-weight':'bold'});
		return;
	}
	$.ajax({
		type: "POST",
		url: "utilities.php",
		data: {op:"Curriculum.getProgrammeCourses", programme_id:programme_id, level:level, semester:semester},
		success: function (response) {
			var data = JSON.parse(response);			
			if(data.response_code == 0){
				myElement.innerHTML = "";
				myElement.innerHTML += "<option value=''>::SELECT::</option>";
				var response_data = data.data;
				if (response_data != null){
					response_data.forEach((element, index) => {
...
$(document).on("change", ".course_code_class", function(){
