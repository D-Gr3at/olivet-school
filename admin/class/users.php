    ...
    public function userlist($data)
    {
		...
		$columner = array(
            ...
            array( 'db' => 'username',   'dt' => 11, 'formatter'=>function($d,$row){
                ...
                {
                    $sack = ($row['status']=="1")?"<span onclick=\"sackUser('$d','$row[status]')\" style='cursor:pointer' class='badge badge-success'>Recall User</span>":"<span onclick=\"sackUser('$d','$row[status]')\" style='cursor:pointer' class='badge badge-primary'>Sack User</span>";
                    return  $sack."<button onclick=\"trigUser('".$d."','".$row['user_disabled']."')\" class='btn btn-sm ".$locking_class."'>".$locking."</button><a class='btn btn-sm btn-warning'   onclick=\"getModal('setup/pastor.php?op=edit&username=".$d."','modal_div')\"  href=\"javascript:void(0)\" data-toggle=\"modal\" data-target=\"#defaultModalPrimary\" >EDIT THIS USER</a>";
                }
                else if($_SESSION['role_id_sess'] == 003)
                {
                    return  "<button onclick=\"trigUser('".$d."','".$row['user_disabled']."')\" class='btn btn-sm ".$locking_class."'>".$locking."</button>&nbsp;|&nbsp;<a class='btn btn-sm btn-warning'   onclick=\"getModal('setup/user_edit.php?op=edit&username=".$d."','modal_div')\"  href=\"javascript:void(0)\" data-toggle=\"modal\" data-target=\"#defaultModalPrimary\" >EDIT THIS USER</a>";
                }
