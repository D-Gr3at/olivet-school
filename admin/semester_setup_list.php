  ...
  <style>
   ...
   <div class="card">
    <div class="card-header">
        ...
        <h6 class="card-subtitle text-muted">The report contains semesters that have been setup in the system.</h6>
    </div>
    <div class="card-body">
     <div class="row">
        <?php
         if($_SESSION['role_id_sess'] == 003 || $_SESSION['role_id_sess'] == 001 || $_SESSION['role_id_sess'] == 005 )
         {
             ?>
         <div class="col-sm-2">
             <a class="btn btn-warning" onclick="getModal('setup/semester_setup.php?op=new','modal_div_lg')"  href="javascript:void(0)" data-toggle="modal" data-target="#sizedModalLg">Create Semester</a>
         </div>
         <?php
         }
         ?>
...
//          d.end_date = $("#end_date").val();
  ...
  function do_filter() {
    ...
    function trigSemester(id,status){
        var r_status = (status == 1)?"Disable":"Enable";
        var cnf = confirm("Are you sure you want to "+r_status+" this semester ?");
        if(cnf)
           {
                $.blockUI();
               $.post('utilities.php',{op:'Semester.changeSemesterStatus',current_status:status,semester_id:id},function(resp)
               {
                   $.unblockUI();
