...
define("ONEPAY_MERCHANTID", "ACC-OPMHT000000235");
...
class dbobject
{
    ...
    public function onePayRequeryCart($transid)
    {
        ...
        if ($err) {
            ...
            $payment_gate = $result->data->payment_gate;
            $payment_ref= $result->data->payment_ref;
            $trans_id=substr($transid, 9, 12);
            $str = "UPDATE app_onepay_transactions SET response_code='$code', response_message='$message', processed_date='$date',payment_gate='$payment_gate',payment_ref='$payment_ref' WHERE merch_trans_id='$trans_id'";
    ...
    //Our custom function.
    public function generatePIN($digits = 4, $type = 0)
    {
...
public function doConfirmPaymentGeneral($rrr)
    ...
    {
        $query_data = "update app_applicant_account_setup set rrr_status='$scode' where rrr='$rrr' ";
    ...
    public function getRRRRemita2($studentName, $phoneno, $Program, $desc = 'olivet', $payerEmail = 'info@olivet.com.ng', $code)
    {
        ...
        $total_amount = 0;
        $count = 0;
        $query_ = "SELECT * FROM app_account_settup WHERE programId= '101'";
        //echo $query_;
      ...
      //  file_put_contents("rmit.txt", $content);
       ...
       // file_put_contents("result1.txt", $json_response);
        ...
        if ($status_code == '025') {
            $now = @date('Y-m-d H:i:s');
            $r_r_r = time();
            $str = "INSERT INTO app_tbl_rrr_log(rrr,email,amount,date_generated,date_paid,order_id,status_code,status_message)VALUE('$RRR','$payerEmail','$total_amount','$now','$now','$orderId','025','$status_msg')";
        ...
        //file_put_contents("rresult.txt", $str);
         ...
         //reg_id, session, email, surname, othernaame, phone_number, program, userpassword, created, rrr, reg_status, rrr_status, linkCode, date_of_birth, gender, marital_status, Nationality, state_of_origin, local_Gov_Area, District_word, tribe, religion, postal_address, exam_center, ip, gname, gaddress, statusCode, pay_status, pbirth, passport, fname, rrr_acceptance, rrr_acceptance_date, rrr_acceptance_status, application_lock, educational_status, admissionstatus, date_adm, exam_center_id
            $str = "UPDATE app_applicant_account_setup SET rrr_acceptance = '$RRR', rrr_acceptance_status = '0' where reg_id='$code'";
...
public function getRRRRemita($studentName, $phoneno, $Program, $desc = 'sonm', $payerEmail = 'info@sonm.com.ng', $code)
    {
        ...
        $total_amount = 0;
        $count = 0;
        $query_ = "SELECT * FROM app_account_settup WHERE programId= '$programId'";
        //echo $query_;
        $result_ = mysql_query($query_);
        while ($row = mysql_fetch_array($result_)) {
            $beneficiaryName = $row['beneficiaryName'];
       ...
       // file_put_contents("result.txt", $json_response);
        ...
        if ($status_code == 25) {
           ...
           // $label = mysql_affected_rows();
            ...
            $str = "UPDATE app_applicant_account_setup SET rrr = '$RRR', rrr_status = '025' where linkCode='$code'";
            //echo $str;
            $result = mysql_query($str); // or die(mysql_error());
            return $RRR;
        } else {
